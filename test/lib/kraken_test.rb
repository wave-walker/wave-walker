...
class KrakenTest < ActiveSupport::TestCase
  ...
  class TradesTest < ActiveSupport::TestCase
    test 'too many request raise an error' do
      stub_request(:get, 'https://api.kraken.com/0/public/Trades?count=1000&pair=XXBTZEUR&since=0')
        .to_return(
          status: 200,
    ...
    test 'invalid asset pair raise an error' do
      stub_request(:get, 'https://api.kraken.com/0/public/Trades?count=1000&pair=FOOBAR&since=0')
        .to_return(
          status: 200,
    ...
    test 'returns the trades and the cursor position' do
      ...
      assert_equal 123_456, response.cursor
    end

    test '#last_page?, is set the last trades are returned' do
      assert_not Kraken::TradesResponse.new(Array.new(Kraken::TRADE_PAGE_SIZE), 0).last_page?
      assert Kraken::TradesResponse.new(Array.new(5), 0).last_page?
    end
  end
end
