...
class OhlcRangeValueTest < ActiveSupport::TestCase
  test '.new, should create range for duration' do
    range = OhlcRangeValue.at(duration: 1.hour, time: Time.zone.at(0))
  ...
  test '.new, should create range at timestamp' do
    range = OhlcRangeValue.at(duration: 4.hours, time: Time.zone.at(3600 * 4))
  ...
  test '.new, should create range for timestamp at any point in range' do
    range = OhlcRangeValue.at(duration: 8.hours, time: Time.zone.at(3600 * 9))
  ...
  test '.new, should create range that dose not include last point' do
    range_h1 = OhlcRangeValue.at(duration: 1.hour, time: Time.zone.at(0))
    range_h2 = OhlcRangeValue.at(duration: 1.hour, time: Time.zone.at(3600))
  ...
  test '#position, should return position' do
    assert_equal OhlcRangeValue.at(duration: 1.hour, time: Time.zone.at(3600)).position, 1
    assert_equal OhlcRangeValue.at(duration: 1.hour, time: Time.zone.at(7200)).position, 2
    assert_equal OhlcRangeValue.at(duration: 1.hour, time: Time.zone.at(11_800)).position, 3
  end
end
