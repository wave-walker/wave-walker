...
class SmoothedMovingAverageTest < ActiveSupport::TestCase
  ...
  test '.bulk_create calls bulk_create_interval for all intervals with given duration' do
    ...
    SmoothedMovingAverage.expects(:bulk_create_interval).with(asset_pair:, duration:, interval: 16)
    SmoothedMovingAverage.expects(:bulk_create_interval).with(asset_pair:, duration:, interval: 19)
    SmoothedMovingAverage.expects(:bulk_create_interval).with(asset_pair:, duration:, interval: 25)
